...
def step_subprocess(name, script, *args, max_retries=3):
    print(f'\n{"="*60}')
    print(f'  STEP: {name}')
    print(f'  Script: {script}')
    print(f'{"="*60}')
    
    # Run as a module, e.g., 'steps.fix_italic_foreign' instead of 'fix_italic_foreign.py'
    module_name = f"steps.{script.replace('.py', '')}"
    ...
    for attempt in range(1, max_retries + 1):
        try:
            # Set PYTHONPATH to SCRIPT_DIR so sub-modules can find 'pipeline' and 'steps' packages
            env = os.environ.copy()
            ...
            if result.returncode == 0:
                ...
                print(f'  ⚠  [Attempt {attempt}/{max_retries}] Step gagal dengan exit code {result.returncode}')
...
def run_pipeline_thesis(doc_path):
    ...
    pipeline = [
        ...
        ('E. Postprocess (ABSTRAK, placeholders, TOC)', 'fix_postprocess.py',
         '--merge-front-matter', str(config.FRONT_MATTER_PATH),
    ...
    print(f'{"="*60}')
    subprocess.run([sys.executable, '-m', 'check.check_pedoman', str(doc_path)], env=env)
