...
try:
    from providers import DEFAULT_OLLAMA_MODEL
except ImportError:
...
class PMBAnalysisPipeline:
    def __init__(self, file_path, llm_provider="Ollama", llm_model=None,
                 cloud_api_key=None, cloud_model=None, ollama_model=None):
        self.file_path = file_path
        self.llm_provider = llm_provider
        self.llm_model = llm_model or DEFAULT_OLLAMA_MODEL
        self.ollama_model = ollama_model or llm_model or DEFAULT_OLLAMA_MODEL
        self.raw = None
        self.by_year = None
        self.cols = None
        self.hs = None
        ...
        # Hybrid pipeline: OpenRouter cloud engine / 9Router local proxy
        _api_key = cloud_api_key or os.getenv("OPENROUTER_API_KEY", "")
        _provider = os.getenv("LLM_PROVIDER", llm_provider or "openrouter").lower()
    ...
    def data_preparation(self):
        ...
        # A3: Cosine similarity ENTITAS IDENTIK (H2 Opsi A) — bukan pasangan acak.
        #   Kelompokkan per sekolah yang muncul DIKEDUA tahun, hitung mean-embedding
        #   profil sekolah per tahun, lalu cosine antar vektor sekolah yang SAMA.
        school_emb = {}
        for y in years:
            school_emb[y] = {}
    ...
    def modeling(self):
        ...
        logger.info("Parameter GMM: covariance_type=diag, reg_covar=1.0 (A4 softening), init_params=k-means++, max_iter=300, n_init=10, random_state=42, tol=1e-3")
        ...
        for idx, y in enumerate(years):
            ...
            for k in range(2, 13):
                ...
                k_progress = progress_base + (k - 2) * (50 / (total_years * 11))
    ...
    def time_series_analysis(self):
        ...
        for i in range(len(years) - 1):
            ...
            is_break = ari_val < 0.30
            self.ari_pairs.append({
                "y1": y1, "y2": y2, "label": f"{y1}→{y2}",
                "ari": rnd(ari_val, 4), "isBreak": is_break,
            ...
            self.jaccard_pairs.append({
                "y1": y1, "y2": y2, "label": f"{y1}→{y2}",
                "jaccard": rnd(jaccard_val, 4), "method": "hungarian_contingency",
            })

            # 3. Centroid drift with Hungarian-matched pairs
            cd = rnd(matched_cost, 4) if matched_cost != float("inf") else float("inf")
            self.centroid_drifts.append({
                "y1": y1, "y2": y2, "label": f"{y1}→{y2}",
                "drift": cd, "method": "hungarian_matched",
            })
        # Forecasting 2025
    ...
    def generate_personas_only(self, provider=None):
        ...
        # Sequential: hindari ThreadPoolExecutor yang broken di Streamlit (ScriptRunContext)
        for task in tasks:
            try:
                self._report_progress(
                    f"Persona {tasks.index(task)+1}/{len(tasks)} (Llama 3.2 lokal)",
                    int((tasks.index(task)+1) / len(tasks) * 100)
                )
                y, ci, persona_text = generate_persona(task)
                if y not in personas:
                    personas[y] = []
                personas[y].append({"cluster": ci, "persona": persona_text})
            except Exception as e:
    ...
    def causal_trend_analysis(self):
        ...
        for i in range(1, len(years)):
            # Jeda antar pair agar tidak mem-burst 9Router (cegah rate-limit 429
            # pada backend Cloudflare proxy). Pair pertama langsung jalan.
            if i > 1:
                time.sleep(2)
            ...
            if self.hybrid and self.hybrid.cloud_available:
                provider_label = "9Router proxy" if (self.hybrid.cloud_engine and self.hybrid.cloud_engine.base_url and "localhost" in self.hybrid.cloud_engine.base_url) else "OpenRouter cloud"
                logger.info(f"[Pipeline] 🌐 Causal analysis {y1}→{y2} via {provider_label}")
    ...
    def narrative_summary(self):
        ...
        if self.hybrid and self.hybrid.cloud_available:
            provider_label = "9Router proxy" if (self.hybrid.cloud_engine and self.hybrid.cloud_engine.base_url and "localhost" in self.hybrid.cloud_engine.base_url) else "OpenRouter cloud"
            logger.info(f"[Pipeline] 🌐 Narrative summary via {provider_label}")
            ari_summary = [
    ...
    def generate_table_narratives(self):
        ...
        if os.path.exists(str(OUTPUTS_DIR / "tabel_4_16_prioritasi_2025.csv")):
            ...
            self.image_narratives["gambar_4_4"] = generate_llm_response(
                prompt, self.llm_provider, None, 2000, model=self.llm_model
            ) or """Gambar 4.4 memvisualisasikan proyeksi pendaftar mahasiswa baru ITSNU Pekalongan untuk tahun 2025 berdasarkan model regresi linier dari data fase Recovery. Garis tren menunjukkan pertumbuhan bertahap dari 2022-2024, dengan titik proyeksi 2025 menandai target yang realistis. Visualisasi ini mendukung perencanaan kapasitas kampus dan alokasi sumber daya untuk menangani tren pendaftaran masa depan."""

        # Narratives for scatter plots per year (Gambar 4.5a, 4.5b, etc.)
        years = sorted(self.by_year.keys())
        for i, y in enumerate(years):
    ...
    def save_outputs(self):
        ...
        # Gambar 4.2 Silhouette Line Chart
        ...
        # Tabel 4.6 ARI, Jaccard, Centroid Drift
        ...
        for y in years:
            ...
            if pts2d is not None:
                ...
                plt.title(
                    f"Gambar 4.5{chr(97 + years.index(y))} – PCA 2D Klaster GMM Tahun {y}"
                )
                plt.xlabel("PC1")
                plt.ylabel("PC2")
