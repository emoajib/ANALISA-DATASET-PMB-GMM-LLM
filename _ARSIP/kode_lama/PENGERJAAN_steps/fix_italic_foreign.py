#!/usr/bin/env python3
"""fix_italic_foreign.py — Italicize foreign terms"""
import os, sys, re
...
TERMS = sorted(['chain of thought', 'gaussian mixture', 'preprocessing', 'embedding', 
    'pipeline', 'reasoning', 'clustering', 'dataset', 'deployment', 'framework',
    'threshold', 'encoder', 'decoder', 'transformer', 'prompt', 'token'], key=len, reverse=True)
...
def in_quotes(t, s, e): return t[:s].count('"') % 2 == 1
...
def set_it(r):
    rp = r.find(qn('w:rPr'))
    if rp is None: rp = etree.SubElement(r, qn('w:rPr'))
    i = rp.find(qn('w:i'))
    if i is None: i = etree.SubElement(rp, qn('w:i'))
    i.set(qn('w:val'), 'true')

def italicize_span(p, cs, ce):
...
for i, p in enumerate(doc.paragraphs):
    if is_heading(p): continue
    txt = p.text
    if not txt or not txt.strip(): continue
    matches = []
    for t in TERMS:
        for m in re.finditer(r'\b' + re.escape(t) + r'\b', txt, re.IGNORECASE):
    ...
    for s, e in matches:
        ...
        if in_quotes(txt, s, e): continue
        filtered.append((s, e))
    if not filtered: continue
    filtered.sort(reverse=True)
    changes = 0
    for s, e in filtered:
        if italicize_span(p, s, e): changes += 1
    if changes:
        print(f"  [ITALIC] Par[{i}] +{changes} term(s)")
        total_changes += changes; total_paras += 1

print(f"\nParagraf diubah: {total_paras}, total istilah: {total_changes}")
doc.save(PATH)
print(f"Saved: {PATH}")
